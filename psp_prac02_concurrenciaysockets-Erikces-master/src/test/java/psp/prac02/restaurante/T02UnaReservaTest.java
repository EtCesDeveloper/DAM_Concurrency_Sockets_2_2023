...
class T02UnaReservaTest {
	...
	static void ejecucion() throws IOException {
		Restaurante deModa = new Restaurante(NUM_SALONES, NUM_PUERTAS, 6100);
		deModa.abrir();
		try (Socket socketPuertas = new Socket("localhost", 6100);
	...
	static void limpieza() {
		...
		fL.renameTo(new File("logs_T02.txt"));
		File fC = new File("cartel.txt");
		fC.renameTo(new File("cartel_T02.txt"));
		Salida.reset();
	}
	
	@Test
	void testSalon() {
		String expected = "Salon 0: 3/10 [Pepe para 3(";
